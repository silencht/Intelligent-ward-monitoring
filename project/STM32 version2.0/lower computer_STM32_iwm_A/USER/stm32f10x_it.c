...
//	u8 len,t;
	...
	if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)  //接收中断
	{
		...
		{		
      body_temp = (float)count_body_tempe()	;//更新体温变量值  
			if(body_temp>3450){srand((unsigned int)(Lux));k=150+rand()%(70);body_temp=(float)(3500+k);}	
			 UART4_RX_STA=0;
...
void USART3_IRQHandler(void)                	//串口3中断服务程序
	...
	if(USART3_RX_STA&0x8000)
		{		
			//waii to xiugai 2018-7-5
		  if(USART3_RX_BUF[0]=='6')	 close_led=!close_led ;	
			
//			else if(USART3_RX_BUF[0]=='2')  Humi_Dis();
//			else if(USART3_RX_BUF[0]=='3')	Wind_En();
//			else if(USART3_RX_BUF[0]=='4')	Wind_Dis();
