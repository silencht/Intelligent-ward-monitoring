...
/* 
@brief得到通道标志位
@param USARTx:参数选择USART外围。
* @param USART_FLAG:指定检查的旗帜。
*该参数可以是下列值之一:
* @arg USART_LSR_RFDR
* @arg USART_LSR_OEI
* @arg USART_LSR_PEI
...
/* @brief启用或禁用USART中断。
* @param USARTx:参数选择USART外围。
* @param USART_IER:指定如果USART中断源被启用或禁用。
*该参数可以是下列值之一:
* @arg USART_IER_MSIE
* @arg USART_IER_RLSIE
...
/* @brief配置Rx FIFO中断触发字节。
* @param USARTx:参数选择USART外围。
* @param USART_RFITL:指定USART Rx FIFO中断触发字节。
*该参数可以是下列值之一:
* @arg USART_RFITL_01
* @arg USART_RFITL_04
* @arg USART_RFITL_08
...
/* @brief USART FIFO超时的值。
* @param USARTx:参数选择USART外围。
* @param USART_TimeOut:指定超时的值。
* @retval没有*/
void USART_SetTimeOutValue(HT_USART_TypeDef* USARTx, u32 USART_TimeOut);

/* @brief清除写和读Tx FIFO或Rx FIFO
* @param USARTx:参数选择USART外围。
* @param USART_FIFODirection:确定TX FIFO或Rx FIFO重置。
*此参数可以任意组合的下列值:
* @arg USART_FIFO_TX:Tx FIFO重置
* @arg USART_FIFO_RX:Rx FIFO重置
* @retval没有*/
