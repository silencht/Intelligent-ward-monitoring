 ...
 *   This parameter can be one of the following values:
 *         @arg USART_IER_MSIE					调制解调器中断使能 (Irpt_MODSI)
 *         @arg USART_IER_RLSIE					接收线路状态中断使能 (Irpt_RLSI)
 *         @arg USART_IER_THREIE				发送器 FIFO 阀值级别中断使能 (Irpt_TFTLI)
 *         @arg USART_IER_RDAIE         接收器 FIFO 阀值级别中断使能 (Irpt_RFTLI) 或接收器 FIFO 超时中断使能 (Irpt_RTOI)
 *         @arg USART_IER_ENON
 * @param NewState: This parameter can be ENABLE or DISABLE.
 * @retval None
 ************************************************************************************************************/
