...
#define INT_READ        GPIO_ReadInBit(HT_GPIOD,GPIO_PIN_4)

/*---------------------自定义MAX30102模块变量-----------------------------*/

	uint32_t aun_ir_buffer[150];   //infrared LED sensor data红外传感器参数寄存
	uint32_t aun_red_buffer[150];  //red LED sensor data红色LED传感器参数寄存
	u8*    zhuan;
	int32_t n_ir_buffer_length; //data length  数据长度

	int32_t n_spo2;         //SPO2 value   血氧参数值
	int8_t  ch_spo2_valid;  //indicator to show if the SPO2 calculation is valid SPO2参数计算是否有效标志位
	int32_t n_heart_rate;   //heart rate value  心率数值
	int8_t  ch_hr_valid;    //indicator to show if the heart rate calculation is valid  心率参数计算是否有效标志位
	uint8_t uch_dummy;  
...
*
* \param[in]    uch_addr    - register address寄存器地址。
* \param[in]    uch_data    - register data寄存器写入数据
*
...
*/
{
    ...
    /* 第9步：读取数据 */
    {
        *puch_data = i2c_ReadByte(NO);	/* 读1个字节 */
...
void MAX30102_INT_LINE(void)
{
  CKCU_PeripClockConfig_TypeDef CKCUClock = {{0}};
	CKCUClock.Bit.PD                    = 1;
  CKCUClock.Bit.BKP                   = 1;
  CKCU_PeripClockConfig(CKCUClock, ENABLE);
	 /* Configure  GPIOD_pin4  as the input function   */
	AFIO_GPxConfig(GPIO_PD, AFIO_PIN_4, AFIO_MODE_DEFAULT); //功能复用默认，即为GPIO模式
	 /* Configure GPIO direction of input pins */
	GPIO_DirectionConfig(HT_GPIOD, GPIO_PIN_4, GPIO_DIR_IN);//设置GPIOD_PIN2引脚为输入模式
	 /* Configure GPIO pull resistor of input pins   配置GPIOD_PIN2,输入引脚为上拉电阻         */
...
void max30102_loop(void)
{
    ...
    //read the first 150 samples, and determine the signal range 读取前面150个数据，然后确定信号的范围
    for(i = 0; i < n_ir_buffer_length; i++)
    {
        while(INT_READ == 1); //wait until the interrupt pin asserts
...
//				if(spo2Avg<60|hrAvg==0){
//				  LCD_ShowString(135,45,230,16,16,"error");
//          LCD_ShowString(135,65,230,16,16,"error"); 
//				}
//				else{
//				LCD_ShowString(135,45,230,16,16,"     ");
//        LCD_ShowString(135,65,230,16,16,"      "); 
				LCD_ShowxNum(15+5*24,45,(float)hrAvg,3,16,0);
				LCD_ShowxNum(15+5*24,65,(float)spo2Avg,3,16,0);
//				}

				u0_printf("The heart rate is %d,SPO0 is %d\r\n-------------------------------------\r\n",hrAvg,spo2Avg);
//    }
}
