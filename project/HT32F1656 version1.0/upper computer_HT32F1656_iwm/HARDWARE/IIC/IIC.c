 ...
 void I2C_GPIO_Config(void)
{
  ...
  CKCUClock.Bit.PB                    = 1;
  CKCUClock.Bit.BKP                   = 1;
  CKCU_PeripClockConfig(CKCUClock, ENABLE);//开启GPIO时钟
	
 	GPIO_InputConfig(HT_GPIOB, GPIO_PIN_12|GPIO_PIN_13,ENABLE);
   GPIO_DirectionConfig(HT_GPIOB, GPIO_PIN_12|GPIO_PIN_13, GPIO_DIR_OUT);//配置引脚方向：输出,B12:SCL  B13:SDA
   GPIO_OpenDrainConfig(HT_GPIOB, GPIO_PIN_12|GPIO_PIN_13, ENABLE);		  //开漏
//   GPIO_PullResistorConfig(HT_GPIOB, GPIO_PIN_6|GPIO_PIN_7, GPIO_PR_UP);//拉电阻
	SCL_H;
	SDA_H;
...
{
	uint16_t i=0;
	SDA_H;	delay_1us(100);        //释放SDA
	SCL_H;  delay_1us(100); 						      //SCL拉高进行采样
...
void I2C_SendByte(u8 dat)
{
  ...
  for (i=0; i<8; i++)         //8位计数器
  {
		...
		delay_1us(100); 
    SCL_H;                //拉高时钟，给从机采样
    delay_1us(100);        //延时保持IIC时钟频率，也是给从机采样有充足时间
    SCL_L;                //拉低时钟，给SDA准备
    delay_1us(100); 		  //延时保持IIC时钟频率
		dat <<= 1;          //移出数据的最高位  
  }					 
}
//**************************************
...
u8 I2C_RecvByte()
{
    ...
    { 
      ...
      SCL_H;                //拉高时钟线，采样从机SDA
		  ...
		  if(SDA_read) //读数据    
		   dat |=0x01;      
       delay_1us(25);     //延时保持IIC时钟频率		
       SCL_L;           //拉低时钟线，处理接收到的数据
       delay_1us(25);   //延时给从机准备SDA时间
    } 
    return dat;
}
//**************************************
